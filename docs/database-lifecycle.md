...
| Vault scan                                     | `prepareDatabaseForUse()` or an owning rebuild workflow invokes `scanVault()` against a physically ready local database                                                          | Complete storage/database convergence, a durable readiness state, or a full event-queue drain; a `false` result prevents the wider transition unless the host asked to complete after failed pairs |
...
8. restore stored storage-event operations when applicable, and scan the Vault, stopping when the scan fails unless the host asked to complete after failed pairs and only pairs failed;
...
A successful scan result means that every selected pair was either `completed` or deliberately `skipped`; it does not mean that every discovered Metadata entry was resolved or that storage and the database are identical. A `false` result, or an exception converted to `false` by the maintained `scanVault()` handler boundary, prevents `onDatabaseInitialised()`, the current batch-wait release, and application readiness, except where the host asks otherwise as follows.

A host may pass `completeAfterFailedPairs` in the `DatabasePreparationOptions` of `initialiseDatabase()`. When the scan then returned its aggregate result and only individual pairs failed, as the outcome of that scan reports, the preparation continues as after a successful scan: it dispatches `onDatabaseInitialised()` with the same error handling, releases the current batch waits, and sets application readiness. The scan result itself stays `false`, and its outcome, including how many pairs failed, reaches the host through `scanOutcome`, which the preparation clears before the scan, so an outcome left by an earlier scan cannot complete a preparation whose scan did not run. A database which is not ready, a scan which could not run, and an error at any point, including one after the aggregate result, still stop the preparation, as does any later phase which fails. Self-hosted LiveSync asks for this at start-up, because a file whose chunks have not arrived yet cannot be written until the synchronisation which fetches them has started; the failed pairs are tried again as [Full offline scan](offline-scanner.md) describes.

The scanner's `initialized` marker means that an earlier permitted invocation reached its aggregate-result boundary. It is recorded after the first aggregate `false` result as well as after `true`, so that the next runtime can restore stored operations; it is not evidence of a successful or fully converged scan. A host may maintain a separate replication-result snapshot. The scan hands the entries it could not write to storage to that queue through `parseSynchroniseResult()`, but the queue itself is outside this Offline Scanner contract.
