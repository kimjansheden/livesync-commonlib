...
7. fill the caller's outcome, when one was given, and return the aggregate pair result.
...
| ----------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Writing a database entry to storage (`update-storage`, or `sync-newer` towards storage), by result or error | `ReplicationService.parseSynchroniseResult()`, when the host provides it                                                                            | When the host's replication-result queue applies it. Self-hosted LiveSync applies it once the plug-in is ready, and keeps an entry whose chunks are still missing for its next synchronisations. |
| Storing a storage file into the database (`update-db`, or `sync-newer` towards the database), by error      | `StorageAccess.appendStorageEvents()` as a `CHANGED` event with `revalidate`, when the storage access provides it                                   | When the storage-event queue runs it, revalidated against current storage like a restored event.                                                                                                 |
| Storing an offline deletion into the database (`delete-db`), by result or error                             | `StorageAccess.appendStorageEvents()` as a `DELETE` event with `revalidate`, dated when the file was last seen, when the storage access provides it | When the storage-event queue runs it; it is applied only while the path is still absent and the deletion is not older than the current revision.                                                 |
| Deleting a local file (`delete-local`), or any other failure                                                | Nothing                                                                                                                                             | By the next full scan.                                                                                                                                                                           |

A file whose chunks have not arrived yet cannot be written, and the synchronisation which brings them may start only after the scan, which is why an entry which could not be written goes to the host's queue instead of waiting for the next scan. When a host lacks the service, or the hand-over fails, the scan logs it and the next full scan tries the pair again. The storage-event queue applies its own settings: while file watching is suspended it drops the events it is given, so those pairs also wait for the next full scan.

Each invocation reports to its own caller. `FullScanOptions.outcome`, and the third argument of `scanVault()`, take an object which receives the number of failed pairs and how many of them each queue took. It is cleared when the invocation starts and filled only once the whole invocation has returned; a scan which the settings do not permit, or an error at any point, including after the aggregate result, leaves it empty, also when the object was used for an earlier scan. `prepareDatabaseForUse()` uses it to tell a preparation whose scan returned with failed pairs from one whose scan could not run or failed, and a concurrent scan cannot change it.
...
Large-pair coverage holds the first large pair until a smaller pair has started beside it, and verifies that no second large pair starts meanwhile. Retry coverage verifies each row of the hand-over table, including a failure which returned `false` and one which threw, that a failed hand-over is logged without failing the scan, and that the aggregate result stays `false`. Outcome coverage verifies that the outcome stays empty for a scan which could not run and after an error which follows the aggregate result, and that two overlapping scans each report their own count.
