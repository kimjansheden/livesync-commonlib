...
export class DurableReplicationCoordinator {
    ...
    private async drain(task: ReplicationTask): Promise<boolean> {
        while (true) {
            ...
            const succeeded = await this.runLeasedTask(task, lease.fencingToken);
            if (!succeeded) {
    ...
    private async run(task: ReplicationTask): Promise<boolean> {
        while (true) {
            if (!this.running) {
    ...
    async enqueue(task: ReplicationTask): Promise<boolean> {
        ...
        const succeeded = await this.run(task);
