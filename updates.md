...
- With Object Storage, a file whose content did not change is no longer stored as a new revision when only its modification time differs, for example after the start-up scan or after this device wrote a received revision under another time. The file handler compares the content whenever the sizes are equal, and records both times as belonging to the same content. After writing a received revision, it records the time storage shows for the file as the time of that revision, so neither the scan nor the storage event of that write sees a newer file.
- With Object Storage, stores of one device no longer branch from the same revision. The file handler stores an ordinary change on the revision it compared the file with, through `storeWithLiveBaseRevision()`, instead of forcing a child of that revision. When another writer advanced the revision meanwhile, it reads the file and the database again and stores once more: on the new revision when this device's provenance records it, and otherwise, when another device's revision arrived meanwhile, as a conflict beside it. A change stored while a received revision has not been reflected yet is kept beside it as well, instead of being stored over it, when provenance records the revision storage shows and the received revision was made on it. Storage which is unchanged since it showed that revision is not stored at all. The full scan and the second check of a file stored while empty store under the lock of the file's storage events, through the new `IFileHandler.storeFileToDBUnderFileEventLock()`. `storeWithLiveBaseRevision()` without a base revision only creates a document.
- With Object Storage, a conservative merge no longer keeps two different insertions at the same place. They are usually successive states of the same typing, which were stacked as extra lines, so the conflict is left to the user. A merge result names the revision it was merged on and the latest times of the two leaves, and `storeContent()` given that target stores it as an unforced child of that revision, which is refused once the revision is no longer a live leaf. Devices which merge the same leaves therefore create the same revision instead of two merges which conflict.
- A synchronisation cycle with an Object Storage remote reads the remote sync parameters once. The epoch check before the transfer uses the parameters the host read at the start of the cycle instead of reading them again, and the journal client keeps them while the configured remote is the same. The milestone is written only when this device's entry in it changes, or to refresh its connection time after ten minutes, instead of in every cycle: the milestone of an Object Storage remote has no document revision, which was taken for a milestone the remote lacked. `ensureRemoteIsCompatible()` takes the refresh interval, which stays one minute for CouchDB.
- `LiveSyncAbstractReplicator.hasUnsentLocalChanges()` tells whether the local database changed since it was last looked through for sending, without a request to the remote. The journal replicator compares the database sequence with its checkpoint; other replicators report that they have changes.
- On the mobile app with Object Storage, a replication lease which another holder held when the process started is taken over at once, instead of waiting up to 45 seconds for it to expire. Such a lease was left by a process the system closed during a cycle. A lease which another holder acquires later is still waited out, and other hosts wait out every lease as before.
- An active remote configuration which cannot be decrypted, because it was encrypted under an earlier configuration passphrase of the device, is recreated from the decrypted connection settings when they hold a connection of the same type, instead of being reported at every start. It is encrypted again when the settings are next saved. Other configurations are left unchanged.
- `ControlService.isApplyingSettings()` tells lifecycle handlers whether the application is suspended and resumed because settings are being applied.
